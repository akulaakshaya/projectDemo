...
public class ForgotController {
	 ...
	 * @PostMapping("/validateOTP")
	 * 
	 * @ResponseBody public String validateOTP(@RequestParam("otp12") String otp) { System.out.println("validate"+otp);
	 * 
	 * if (otp.equals(generateotp)) { java.sql.Timestamp timestamp1 = cust.getCustLastLoginDate();
	 * java.time.LocalDateTime t1 = timestamp1.toLocalDateTime(); java.time.LocalDateTime t2 =
	 * java.time.LocalDateTime.now(); java.time.Duration duration = java.time.Duration.between(t1, t2);
	 * System.out.println("t1:"+t1); System.out.println("t2:"+t2); long hours = duration.toHours(); long minutes =
	 * duration.toMinutes(); long seconds = duration.getSeconds(); System.out.println("seconds:"+duration.getSeconds());
	 * if(duration.getSeconds()<=30) { return "valid"; } else return "no"; } else { // OTP is invalid return "invalid";
	 * } }
	 * 
	 */
	@PostMapping("/validateOTP")
