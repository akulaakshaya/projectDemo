...
public class adminOrderController {
	...
	public String generateOrderListTable(List<orderModel> orders) {
        ...
        .append("<select id=\"dateRangeFilter\">\n")
        ...
        .append("<select id=\"processedStatusFilter\">\n")
        .append("<option value=\"\">All</option>\n")
		...
		for (orderModel order : orders) {
			...
			if (order.getOrdr_processedby() == null) {
			    htmlContent.append("<button  id=\"red-button\" class=\"btn btn-danger\" data-order-id=\"").append(order.getId())
			        .append("\">Unprocessed</button>");
			    
			} else {
			    htmlContent.append("<button class=\"btn ").append(buttonColor).append("\" data-order-id=\"").append(order.getId()).append("\">")
			        .append(order.getOrdr_processedby()).append("</button>");
			}
	...
	public String loadOrders(@RequestParam(value = "selectDateFilter") String selectDateFilter, Model model) {
	    ...
	    if (selectDateFilter.equals("daily")) {
	        ...
	        startDate = currentDate.withHour(0).withMinute(0).withSecond(0).minusDays(currentDate.getDayOfWeek().getValue() - 1);
