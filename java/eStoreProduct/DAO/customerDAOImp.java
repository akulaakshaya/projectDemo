...
public class customerDAOImp implements customerDAO {
	...
	private final String GET_TOP_CUST_ID="select cust_id from slam_customers order by cust_id desc limit 1";
	private final String SQL_INSERT_CUSTOMER = "insert into slam_customers(cust_id,cust_name,  cust_mobile , cust_regdate ,cust_location , cust_email ,cust_address, cust_pincode , cust_saddress,cust_spincode,cust_status ,cust_lastlogindate, cust_password ) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean createCustomer(custCredModel ccm) {
		System.out.println( "spincode:"+ccm.getCustSpincode());
		 int c_id = jdbcTemplate.queryForObject(GET_TOP_CUST_ID, int.class);
		 c_id = c_id + 1;
		return jdbcTemplate.update(SQL_INSERT_CUSTOMER,c_id, ccm.getCustName(), ccm.getCustMobile(), ccm.getCustRegDate(),
